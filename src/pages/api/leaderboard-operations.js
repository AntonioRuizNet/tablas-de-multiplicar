    ...
    try {
      result = await db.query(`
        ...
        JOIN (
          ...
          UNION ALL SELECT ('a-'||id::text) AS id,user_id FROM activity_operations
          UNION ALL SELECT ('s-'||id::text) AS id,user_id FROM addition_operations
          UNION ALL SELECT ('m-'||id::text) AS id,user_id FROM arithmetic_operations
          UNION ALL SELECT ('c-'||q.id::text) AS id,a.user_id
            FROM challenge_attempt_questions q
            JOIN challenge_attempts a ON a.id=q.attempt_id
            WHERE q.answered_at IS NOT NULL
        ) o ON o.user_id=u.id
        GROUP BY u.id,u.name,u.avatar_icon,u.avatar_color
        HAVING COUNT(o.id)>0
