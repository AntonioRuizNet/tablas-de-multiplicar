    ...
    <main className={styles.page}>
      ...
      {isTeacher && <>
        ...
        {data && selectedRoom && <>
          ...
          <section className={styles.card}><h2>Atención recomendada</h2>{attention.length ? <div className={styles.people}>{attention.map((student) => <article key={student.id}><UserAvatar icon={student.avatar_icon} color={student.avatar_color} size={44} /><div><strong>{student.name || "Jugador"}</strong><p>{student.inactive ? "Sin actividad reciente" : `${student.accuracy}% de aciertos en el periodo`}</p><Link href={`/historial?userId=${student.id}`}>Ver historial</Link></div><span>{student.needs_attention ? "Reforzar" : "Inactivo"}</span></article>)}</div> : <p>No hay alumnos que requieran atención según los datos de este periodo.</p>}</section>
          ...
          <section className={styles.card}><h2>Todos los alumnos</h2><div className={styles.tableWrap}><table><thead><tr><th>Alumno</th><th>Operaciones</th><th>Aciertos</th><th>Última actividad</th><th>Estado</th><th></th></tr></thead><tbody>{data.students.map((student) => <tr key={student.id}><td><span className={styles.student}><UserAvatar icon={student.avatar_icon} color={student.avatar_color} size={32} />{student.name || "Jugador"}</span></td><td>{student.operations}</td><td>{student.accuracy}%</td><td>{student.last_activity ? new Date(student.last_activity).toLocaleDateString("es-ES") : "Sin actividad"}</td><td>{student.needs_attention ? "⚠️ Reforzar" : student.inactive ? "💤 Inactivo" : "✅ Bien"}</td><td><Link href={`/historial?userId=${student.id}`}>Historial</Link></td></tr>)}</tbody></table></div></section>
        </>}
      </>}
    </main>
  </ResourceLayout>;
