      ...
      {error ? <section className={styles.card}><h1>Jugador no encontrado</h1><p>{error}</p></section> : !player ? <section className={styles.card}><p>Cargando jugador…</p></section> : <section className={styles.card}>
        <div className={styles.hero}>
          <div className={styles.playerIdentity}>
            <UserAvatar icon={player.avatarIcon} color={player.avatarColor} size={78} />
            <div><span className={styles.eyebrow}>Perfil público</span><h1>{player.name}</h1><p>{player.rankName} · Nivel {player.level}</p></div>
          </div>
          <strong className={styles.points}>{player.points} pts</strong>
        </div>
